...
<template>
  <RouterLink v-if="isRouterLink && to" :to="to" :class="computedClasses" :aria-disabled="isDisabled"
    :tabindex="isDisabled ? -1 : undefined" @click="handleClick">
    <svg v-if="loading" class="animate-spin -ml-0.5 size-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none"
      viewBox="0 0 24 24">
  ...
  <a v-else-if="isExternalLink && href" :href="href" :target="target" :class="computedClasses"
    :aria-disabled="isDisabled" :tabindex="isDisabled ? -1 : undefined" @click="handleClick">
    <svg v-if="loading" class="animate-spin -ml-0.5 size-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none"
      viewBox="0 0 24 24">
      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
      <path class="opacity-75" fill="currentColor"

<template>
  <Card title="Grados Académicos" :class="{ 'flex-1': grados && grados.length }">
    <template #icon>
    ...
    <div v-if="grados && grados.length" class="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800 -mx-1 px-1">
      <div v-for="(grado, index) in grados" :key="grado.id ?? index" class="group flex items-center gap-2.5 py-2.5 hover:bg-gray-50 dark:hover:bg-white/2 rounded-lg px-2 transition-colors">

  ...
  <Modal :isOpen="isOpen" :title="isEdit ? 'Editar Información Bancaria' : 'Agregar Información Bancaria'" @close="close">
    <form @submit.prevent="guardar" class="space-y-4">
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div class="sm:col-span-2">
          <label class="form-label dark:text-white/70">Número de Cuenta</label>
          <input
            type="text"
        ...
        <div>
          <label class="form-label dark:text-white/70">Tipo de Cuenta</label>
          <select
        ...
        <div>
          <label class="form-label dark:text-white/70">Banco</label>
          <select
            v-model="form.banco"
            class="mt-1 block w-full rounded-md border px-3 py-2 shadow-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary dark:bg-slate-800 dark:text-white"
        ...
        <div class="sm:col-span-2">
          <label class="form-label dark:text-white/70">CCI (Código de Cuenta Interbancaria)</label>
          <input
            type="text"
            v-model="form.cci"
            maxlength="20"
    ...
    <template #footer>
      ...
      <button
        type="button"
        @click="guardar"
        class="inline-flex w-full justify-center rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-primary/90 sm:ml-3 sm:w-auto">

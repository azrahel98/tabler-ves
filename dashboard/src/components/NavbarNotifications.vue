...
function handleNotificationClick(notif: { id: number; enlace?: string | null; leido: boolean; dni?: string | null }) {
  if (!notif.leido) {
...
<template>
  <div class="relative">
    <button type="button"
      ...
      aria-label="Abrir panel de notificaciones" :aria-expanded="isOpen" @click="toggleDropdown">
      <IconBell class="size-4" :stroke-width="2" />
      <span v-if="notifStore.totalNoLeidas > 0"
      ...
      <span v-else-if="notifStore.isConnected" class="absolute top-2 right-2 size-1.5 rounded-full bg-emerald-500"
        title="Canal en vivo conectado"></span>
    </button>
    ...
    <Transition enter-active-class="transition ease-out duration-200"
      ...
      <div v-if="isOpen"
        ...
        <div class="max-h-96 overflow-y-auto divide-y divide-border/60">
          <div v-if="notifStore.isLoading && notifStore.notificaciones.length === 0" class="p-4 space-y-3">
          ...
          <div v-for="item in notifStore.notificaciones" v-else :key="item.id" :class="[
            ...
            <span v-if="!item.leido" class="absolute left-1.5 top-5 size-1.5 rounded-full bg-primary"></span>

            <div class="relative shrink-0 mt-0.5">

              <img v-auth-src="getPersonalAvatarUrl(item.dni)" alt="Foto del servidor"
            ...
            <div class="absolute right-2 top-3 opacity-0 group-hover:opacity-100 transition">
              <button v-if="!item.leido" type="button"
        ...
        <div class="p-2 border-t border-border bg-card flex items-center justify-between text-[11px]">
          <button type="button"
            ...
            <IconRefresh :class="['size-3.5', notifStore.isLoading ? 'animate-spin' : '']" :stroke-width="2" />
            <span>Actualizar</span>
          </button>

          <span class="text-muted-foreground text-[10px]">
            {{ notifStore.notificaciones.length }} {{ notifStore.notificaciones.length === 1 ? 'notificación' :
              'notificaciones' }}
          </span>
        </div>
      </div>

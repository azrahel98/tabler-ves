<template>
  <header class="sticky top-0 z-99999 flex w-full border-gray-100 bg-card lg:border-b dark:border-white/6 dark:bg-gray-950">
    <div class="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
      <div class="flex w-full items-center justify-between gap-2 border-b border-gray-100 px-3 py-3 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4 dark:border-white/6">
        <button
          :class="sidebarToggle ? 'lg:bg-transparent dark:lg:bg-transparent bg-primary/10 dark:bg-primary/20' : ''"
          class="z-99999 flex h-10 w-10 items-center justify-center rounded-lg border-gray-100 text-gray-500 lg:h-11 lg:w-11 lg:border dark:border-white/6 dark:text-gray-400"
        ...
        <button
          class="z-99999 flex h-10 w-10 items-center justify-center rounded-lg text-gray-700 hover:bg-primary/10 lg:hidden dark:text-gray-400 dark:hover:bg-white/5"
          :class="menuToggle ? 'bg-primary/10 dark:bg-white/5' : ''"
      ...
      <div :class="menuToggle ? 'flex' : 'hidden'" class="shadow-theme-md w-full items-center justify-between gap-4 px-5 py-4 lg:flex lg:justify-end lg:px-0 lg:shadow-none">
        <div class="2xsm:gap-3 flex items-center gap-2">
          <button
            class="hover:text-dark-900 relative flex h-11 w-11 items-center justify-center rounded-full border border-gray-100 bg-card text-gray-500 transition-colors hover:bg-primary/10 hover:text-primary dark:border-white/6 dark:bg-white/5 dark:text-gray-400 dark:hover:bg-white/10 dark:hover:text-white"
        ...
        <div class="relative" ref="userDropdownRef">
          <a class="flex items-center text-gray-700 dark:text-gray-400" href="#" @click.prevent="userDropdownOpen = !userDropdownOpen">
            <span class="text-body-medium mr-1 block"> {{ autenticacionStore.usuario?.nombre || 'Usuario' }} </span>
          ...
          <div
            ...
            <div>
              <span class="text-body-normal block font-semibold text-gray-800 dark:text-white/90 px-3 py-2"> {{ autenticacionStore.usuario?.nombre || 'Usuario' }} </span>
            ...
            <button
              @click="handleLogout"
              class="group text-body-medium mt-3 flex items-center gap-3 rounded-lg px-3 py-2 text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
...
<script setup lang="ts">
  ...
  const { menuLateralAbierto: sidebarToggle, modoOscuro: darkMode } = storeToRefs(configuracionStore)

  const handleLogout = async () => {
    autenticacionStore.cerrarSesion()

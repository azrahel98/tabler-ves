...
### `POST /dash/personal_activo_area`
...
| ---------------- | ------ | --------- |
| `vinculo_id`     | number | sí        |
| `nueva_area_id`  | number | sí        |
| `fecha_cambio`   | string | sí (YYYY-MM-DD) |
| `documento`      | object | sí        |

**Estructura de `documento`** (igual que en otros endpoints):
...
### `POST /personal/activos_por_distrito`
...
Sube un archivo y lo vincula a múltiples trabajadores creando un documento de legajo único.
...
```json
{
  "message": "Documento registrado y vinculado correctamente...",
  "documento_id": 123,
  "file_hash": "uuid...",
  "count": 5
}
...
| --------------- | ------- | --------- |
| `dni_asociado`  | string  | sí        |
| `original_name` | string  | sí (debe terminar en `.pdf`) |
| `external_url`  | string  | sí (debe empezar con `https://`) |
| `documento_id`  | number? | no        |
...
- `400` si la URL no comienza con `https://`, si el nombre no termina en `.pdf`, o si falta `dni_asociado`.
...
```json
[{ "id": 1, "original_name": "mi_doc.pdf", "file_hash": "uuid...", "extension": "pdf", "external_url": null, "usuario_subida": "Admin", "fecha_subida": "2024-01-15 10:30:00" }]
```

...
use crate::common::errors::{ApiError, validar};
use crate::common::jwt::Claims;
use crate::personal::models::{
    ContactoEmergencia, DatosBancarios, DatosBancariosResponse, Documento, DocumentoSindicato,
    EventoVinculoPayload, GradoAcademico, NuevoVinculo, Perfil, Vinculos, es_dni_valido,
};
use crate::personal::service::{
...
pub async fn editar_perfil(
    ...
    validar(&perfil.0)?;
    use crate::common::audit::AuditContext;
    let audit_ctx = AuditContext::from_request(&req);
    let rows_affected = personal_service::update_perfil(&data.db, &perfil.0, &audit_ctx).await?;
...
pub async fn obtener_documento_por_id(
    ...
    Ok(HttpResponse::Ok().json(doc_entity))
}
...
pub async fn crear_documento(
    ...
    validar(&body.0)?;
    let new_id = documento_service::crear_documento(&data.db, &body.documento).await?;
...
pub async fn registrar_trabajador(
    ...
    validar(&body.0)?;
    let key = crate::common::audit::get_db_key();
...
pub async fn vinculos_por_dni(
    ...
    let dni = path.into_inner();
    let (mut vinculos, sunat_vinculos) = tokio::try_join!(
        personal_service::get_vinculos_by_dni(&data.db, &dni),
        personal_service::get_sunat_vinculos_by_dni(&data.db, &dni),
    )?;

    let eventos_futures: Vec<_> = vinculos
        .iter()
        .map(|v| personal_service::get_eventos_by_vinculo_id(&data.db, v.id))
        .collect();
    let eventos_list = futures_util::future::try_join_all(eventos_futures).await?;

    for (v, evs) in vinculos.iter_mut().zip(eventos_list) {
        if v.origen.is_empty() {
            v.origen = "".to_string();
        }
        v.eventos = evs;
    }

    let mut sunat_mapped: Vec<Vinculos> = sunat_vinculos
        .into_iter()
        .map(|s| {
            let estado = if s.fecha_cese.is_some() {
                "inactivo".to_string()
            } else {
                "activo".to_string()
            };
            Vinculos {
                ...
                doc_ingreso_id: None,
                doc_ingreso: None,
                numero_doc_ingreso: None,
                descrip_ingreso: None,
                fecha_ingreso: Some(s.fecha_inicio),
                area: None,
                cargo: "".to_string(),
                regimen: None,
                sueldo: None,
                codigo: None,
                cargo_estructural: None,
                grupo_ocupacional: None,
                estado,
                doc_salida_id: None,
                doc_salida: None,
                descrip_salida: None,
                fecha_salida: s.fecha_cese,
                numero_doc_salida: None,
                sindicato: None,
                eventos: Vec::new(),
                origen: "SUNAT".to_string(),
            }
        })
        .collect();

    vinculos.append(&mut sunat_mapped);
    vinculos.sort_by(|a, b| b.fecha_ingreso.cmp(&a.fecha_ingreso));

    Ok(HttpResponse::Ok().json(vinculos))
}

pub async fn eventos_por_vinculo(
    data: web::Data<AppState>,
    path: web::Path<i32>,
) -> Result<impl Responder, ApiError> {
    let vinculo_id = path.into_inner();
    let eventos = personal_service::get_eventos_by_vinculo_id(&data.db, vinculo_id).await?;
...
pub async fn renuncia_por_vinculo(
    ...
    if let Some(dni_str) = json_value.get("dni").and_then(|d| d.as_str()) {
        ...
        let nombre_completo = persona_row
            .as_ref()
            .and_then(|r| {
                r.try_get::<Option<String>, _>("nombre_completo")
                    .ok()
                    .flatten()
            })
        ...
        let notif_id = crate::notificaciones::repo::crear_notificacion(
            &data.db,
            "RENUNCIA",
            &titulo,
            &mensaje,
            Some(json_value.get("dni").unwrap().as_str().unwrap().to_string()),
            Some(&metadata_obj),
        )
        .await
        .unwrap_or(0);

        let evento = crate::notificaciones::models::NotificacionEvento {
            id: notif_id,
            tipo: "RENUNCIA".to_string(),
            titulo,
            mensaje,
            dni: Some(json_value.get("dni").unwrap().as_str().unwrap().to_string()),
            leido: false,
...
pub async fn upsert_evento_vinculo(
    ...
    if let Some(ref doc_inicio) = payload.documento_inicio {
        ...
        let doc_salida_final = if payload.mismo_documento.unwrap_or(false) {
            Some(doc_id)
        } else {
            None
        };
        ...
        sqlx::query(
            r#"
            INSERT INTO eventovinculo (vinculo_id, tipo_evento, nueva_area_id, nuevo_cargo_id, documento_inicio, documento_salida, estado)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            "#,
        )
            ...
            sqlx::query("UPDATE vinculo SET estado = 'pendiente' WHERE id = ?")
                ...
                .map_err(|e| {
                    ApiError::InternalError(format!(
                        "Error al actualizar estado del vínculo a pendiente: {}",
                        e
                    ))
                })?;
        }
        ...
        let ev_row = sqlx::query!(
            "SELECT vinculo_id, tipo_evento, documento_inicio FROM eventovinculo WHERE id = ?",
            evento_id
        )
        .fetch_optional(&mut *tx)
        .await
        .map_err(|e| ApiError::InternalError(format!("Error buscando evento: {}", e)))?
        .ok_or_else(|| ApiError::NotFound("Evento de vínculo no encontrado".into()))?;
        ...
        let doc_salida_id = if let Some(ref doc_salida) = payload.documento_salida {
            ...
            .last_insert_id() as i32
        } else if payload.mismo_documento.unwrap_or(false) {
            ev_row.documento_inicio
        } else {
            return Err(ApiError::BadRequest(
                "Se debe enviar documento_salida o marcar mismo_documento = true para cerrar el evento".into(),
            ));
        };
        ...
        let tipo_evento_actual = ev_row.tipo_evento;
        let vinculo_id_actual = ev_row.vinculo_id;

        if tipo_evento_actual == "abandono" {
            // Resolución de destitución: finaliza el vínculo a inactivo
            sqlx::query("UPDATE vinculo SET estado = 'inactivo', doc_salida_id = ? WHERE id = ?")
                .bind(doc_salida_id)
            ...
            .execute(&mut *tx)
            .await;
        } else if tipo_evento_actual == "destaque" {
            // Fin de destaque: el trabajador reingresa y vuelve a activo

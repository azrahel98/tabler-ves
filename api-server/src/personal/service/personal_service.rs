...
pub async fn get_sunat_vinculos_by_dni(
    db: &MySqlPool,
    dni: &str,
) -> Result<Vec<SunatVinculo>, ApiError> {
    personal_repo::get_sunat_vinculos_by_dni(db, dni)
        .await
        .map_err(|e| {
            eprintln!("Database error: {:?}", e);
            ApiError::InternalError("Error al obtener vínculos de SUNAT".into())
        })
}
pub async fn get_eventos_by_vinculo_id(
    db: &MySqlPool,
    vinculo_id: i32,
) -> Result<Vec<EventoVinculoDetalle>, ApiError> {
    personal_repo::get_eventos_by_vinculo_id(db, vinculo_id)
        .await
        .map_err(|e| {
            eprintln!("Database error: {:?}", e);
            ApiError::InternalError("Error al obtener eventos del vínculo".into())
        })
}
use serde_json::json;

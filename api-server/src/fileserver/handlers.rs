...
SELECT
  di.id,
  CONCAT(tdi.nombre, ' N° ', COALESCE(di.numero, ''), '-', COALESCE(di.year, ''), CASE WHEN COALESCE(dia.sigla, eva.sigla, va.sigla) IS NOT NULL THEN CONCAT('-', COALESCE(dia.sigla, eva.sigla, va.sigla)) ELSE '' END) AS descripcion_doc,
  di.fecha,
  di.descripcion
FROM
  vinculo v
  INNER JOIN eventovinculo ev ON ev.vinculo_id = v.id
  LEFT JOIN documento di      ON di.id = ev.documento_inicio
  LEFT JOIN tipo_documento tdi ON tdi.id = di.tipo_documento_id
  LEFT JOIN area dia          ON dia.id = di.area_id
  LEFT JOIN area eva          ON eva.id = ev.nueva_area_id
  LEFT JOIN area va           ON va.id = v.area_id
WHERE
  v.dni = ?
  AND di.id IS NOT NULL and di.numero is not null
UNION
SELECT
  des.id,
  CONCAT(tdes.nombre, ' N° ', COALESCE(des.numero, ''), '-', COALESCE(des.year, ''), CASE WHEN COALESCE(desa.sigla, eva.sigla, va.sigla) IS NOT NULL THEN CONCAT('-', COALESCE(desa.sigla, eva.sigla, va.sigla)) ELSE '' END) AS descripcion_doc,
  des.fecha,
  des.descripcion
FROM
  vinculo v
  INNER JOIN eventovinculo ev ON ev.vinculo_id = v.id
  LEFT JOIN documento des     ON des.id = ev.documento_salida
  LEFT JOIN tipo_documento tdes ON tdes.id = des.tipo_documento_id
  LEFT JOIN area desa         ON desa.id = des.area_id
  LEFT JOIN area eva          ON eva.id = ev.nueva_area_id
  LEFT JOIN area va           ON va.id = v.area_id
WHERE
  v.dni = ?
  AND des.id IS NOT NULL and des.numero is not null
...
pub async fn upload_batch(
        ...
        match field_name.as_str() {
            ...
            "file" | "archivo" => {
                ...
                while let Some(chunk) = field.try_next().await.map_err(|e| {
                    ApiError::InternalError(format!("Error reading file chunk: {}", e))
                })? {
                    current_size += chunk.len();
                    if current_size > max_file_size {
                        let _ = std::fs::remove_file(&file_path);
                        return Err(ApiError::InternalError(
                            "El archivo no debe pesar más de 20 MB".to_string(),
                        ));
                    }
                    f.write_all(&chunk).map_err(|e| {
                        ApiError::InternalError(format!("Error writing chunk to file: {}", e))
                    })?;
                }
    ...
    let (orig_name, hash, ext) = saved_file
        .ok_or_else(|| ApiError::InternalError("No se recibió ningún archivo".to_string()))?;
    ...
    if dnis_count == 0 {
        return Err(ApiError::InternalError(
            "Debe proporcionar al menos un DNI".to_string(),
        ));
    }
    let mut tx = data
        .db
    ...
    tx.commit()
        .await
        .map_err(|e| ApiError::InternalError(format!("Transaction commit error: {}", e)))?;

...
use crate::dash::service as dash_service;
...
pub async fn reporte_documentos(pool: web::Data<MySqlPool>) -> Result<HttpResponse, ApiError> {
    let result = dash_service::tipos_documento(&pool).await?;
    Ok(HttpResponse::Ok().json(result))
}

#[get("/tipos_documento")]
pub async fn tipos_documento(pool: web::Data<MySqlPool>) -> Result<HttpResponse, ApiError> {
    let result = dash_service::tipos_documento(&pool).await?;
    Ok(HttpResponse::Ok().json(result))
}

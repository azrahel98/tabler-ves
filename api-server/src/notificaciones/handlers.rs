...
use crate::notificaciones::models::{ListarNotificacionesResponse, ListarQuery};
use crate::notificaciones::repo as notificacion_repo;
...
pub async fn stream_notificaciones(
    ...
    let sse_stream = stream::unfold(rx_client, |mut rx| async move {
        rx.recv()
            .await
            .map(|bytes| (Ok::<web::Bytes, actix_web::Error>(bytes), rx))
    });
...
pub async fn listar_notificaciones(
    ...
    let (items, no_leidas) = notificacion_repo::listar_recientes(&data.db, limit)
        .await
...
pub async fn marcar_leida(
    ...
    notificacion_repo::marcar_leida(&data.db, id)
        .await
...
pub async fn marcar_todas_leidas(data: web::Data<AppState>) -> Result<impl Responder, ApiError> {
    let filas = notificacion_repo::marcar_todas_leidas(&data.db)
        .await
        .map_err(|e| {
            ApiError::InternalError(format!("Error al marcar todas las notificaciones: {}", e))
        })?;

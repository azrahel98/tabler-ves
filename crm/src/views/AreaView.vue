  ...
  <div class="px-4 py-5 md:px-6 md:py-6 space-y-5 max-w-[1600px] mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          <div>
            <div class="flex items-center gap-2">
              <h1 class="text-sm sm:text-base font-bold uppercase tracking-wider text-gray-900 dark:text-white">
                {{ identificadorArea || 'Área Municipal' }}
              </h1>
              <span
                v-if="!estaCargando"
                ...
                {{ totalArea }} trabajadores
              </span>
            </div>
            <p class="text-2xs font-medium text-gray-400">
              Detalle de dotación, cargos y regímenes asignados a esta dependencia
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2">
        <button
          type="button"
          @click="cargarArea(identificadorArea)"
    ...
    <div v-if="estaCargando && totalArea === 0" class="space-y-5 animate-pulse">
    ...
    <template v-else>
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div
          ...
          <div class="flex items-center justify-between">
            <div
              class="flex h-7 w-7 items-center justify-center rounded-full bg-purple-50 text-purple-600 dark:bg-purple-950/50 dark:text-purple-400">
              <IconBriefcase class="h-3.5 w-3.5" />
            </div>
            <span class="text-xl sm:text-2xl font-bold tracking-tight text-purple-700 dark:text-purple-400 tabular-nums leading-none">
              {{ regimenPrincipal ? regimenPrincipal.cantidad : 0 }}
            </span>
          </div>
          <div class="mt-3">
            <span class="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-200 block truncate">
            ...
            <span class="text-2xs text-purple-600/80 dark:text-purple-400/80 font-medium block truncate mt-0.5">
              {{ regimenPrincipal?.nombre || 'Sin registros' }}
            </span>
          </div>
        </div>
        ...
        <div
          ...
          <div>
            ...
            <div v-if="distribucionRegimenes.length === 0" class="py-8 text-center text-xs text-gray-400">
              No hay regímenes registrados en esta área
            </div>
              ...
              <RouterLink
                ...
                <div class="flex items-center justify-between text-2xs">
                  ...
                  <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-gray-400">({{ Math.round((regimen.cantidad / (totalArea || 1)) * 100) }}%)</span>
                  </div>
                </div>
              ...
              <div
                ...
                <div class="flex items-center justify-between text-2xs">
                  ...
                  <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-gray-400">({{ Math.round((cargo.cantidad / (totalArea || 1)) * 100) }}%)</span>
                  </div>
                </div>
                <div class="h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                  <div
      ...
      <TablaTrabajadores
        :trabajadores="trabajadoresArea"
        titulo="Personal Asignado"
        :subtitulo="`Trabajadores activos en ${identificadorArea}`"
        :mostrar-filtro-regimen="true"
        :mostrar-columna-area="false" />
    </template>
  </div>
</template>
